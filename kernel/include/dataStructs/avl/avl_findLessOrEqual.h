#ifndef CLIB_TEMPLATE_DATA_STRUCTURES_AVL_FIND_LESS_OR_EQUAL
#define CLIB_TEMPLATE_DATA_STRUCTURES_AVL_FIND_LESS_OR_EQUAL
...
#define DECLARE_AVL_FUNC_FIND_LESS_OR_EQUAL(type, name)                         \
clib_error_code_t AVLFunc(name, findLessOrEqual) (                              \
    const AVLStruct(name) *a_avl,                                               \
    const type *a_value,                                                        \
    const AVLNodeStruct(name) **a_res                                           \
);

#define IMPLEMENT_AVL_FUNC_FIND_LESS_OR_EQUAL(type, name)                       \
clib_error_code_t AVLFunc(name, findLessOrEqual) (                              \
    const AVLStruct(name) *a_avl,                                               \
    const type *a_value,                                                        \
    const AVLNodeStruct(name) **a_res)                                          \
    ...
    *a_res = CLIB_NULLPTR;                                                      \
    ...
    if (a_avl->root == CLIB_NULLPTR) {                                          \
        return CLIB_ERROR_NOT_FOUND;                                            \
                                                                                ...
                                                                                \
    AVL_TYPE_SCORE_LESS_OR_EQUAL score;                                         \
    AVL_TYPE_SCORE_LESS_OR_EQUAL_GET_SCORE(                                     \
        ...
        (&score)                                                                \
                                                                                ...
                                                                                \
    *a_res = AVLNodeFunc(name, findLessOrEqual) (                               \
        a_avl->root,                                                            \
        a_value,                                                                \
        &score                                                                  \
    );                                                                          \
...
#endif // end if CLIB_TEMPLATE_DATA_STRUCTURES_AVL_FIND_LESS_OR_EQUAL
