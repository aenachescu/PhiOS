...
typedef clib_bool_t (*UTDataAVLFindIfCbk_t)(const Data *a_value,
                                            void *a_context);
typedef clib_bool_t (*UTDataAVLRemoveIfCbk_t)(const Data *a_value,
                                              void *a_context);
...
clib_error_code_t UTDataAVL_findLessOrEqual(const UTDataAVL *a_avl,
                                            const Data *a_value,
                                            const UTDataAVLNode **a_res);
clib_error_code_t UTDataAVL_findNearOrEqual(const UTDataAVL *a_avl,
                                            const Data *a_value,
                                            const UTDataAVLNode **a_res);
...
clib_error_code_t UTDataAVL_removeIf(UTDataAVL *a_avl,
                                     UTDataAVLRemoveIfCbk_t a_cbk,
                                     void *a_context, UTDataAVLNode **a_result);
...
static const UTDataAVLNode *
UTDataAVLNode_findNearOrEqual(const UTDataAVLNode *a_parent,
                              const Data *a_value,
                              unsigned int *a_currentScore) {
  if (((*a_currentScore) == 0)) {
    return a_parent;
  ...
  unsigned int score;
  const UTDataAVLNode *result = a_parent;
  if (a_parent->left != ((void *)0)) {
    (&a_parent->left->data)->start <= a_value->start
        ? (*(&score) = a_value->start - (&a_parent->left->data)->start)
        : (*(&score) = (&a_parent->left->data)->start - a_value->start);
    result = UTDataAVLNode_findNearOrEqual(a_parent->left, a_value, &score);
    if (((*a_currentScore) < (*(&score)))) {
      result = a_parent;
    } else {
...
static UTDataAVLNode *UTDataAVLNode_removeIf(UTDataAVLNode *a_parent,
                                             UTDataAVLRemoveIfCbk_t a_cbk,
                                             void *a_context,
                                             UTDataAVLNode **a_result) {
  if (a_parent->left != ((void *)0)) {
    a_parent->left =
        UTDataAVLNode_removeIf(a_parent->left, a_cbk, a_context, a_result);
    if (*a_result != ((void *)0)) {
      return UTDataAVLNode_balance(a_parent);
    }
  }
  ...
  if (a_parent->right != ((void *)0)) {
    a_parent->right =
        UTDataAVLNode_removeIf(a_parent->right, a_cbk, a_context, a_result);
    if (*a_result != ((void *)0)) {
      return UTDataAVLNode_balance(a_parent);
    }
  }
...
clib_error_code_t UTDataAVL_findNearOrEqual(const UTDataAVL *a_avl,
  ...
  *a_res = ((void *)0);
  ...
  (&a_avl->root->data)->start <= a_value->start
      ? (*(&score) = a_value->start - (&a_avl->root->data)->start)
      : (*(&score) = (&a_avl->root->data)->start - a_value->start);
  *a_res = UTDataAVLNode_findNearOrEqual(a_avl->root, a_value, &score);
  return ((clib_error_code_t)0);
}
clib_error_code_t UTDataAVL_foreachInorder(const UTDataAVL *a_avl,
                                           UTDataAVLForeachCbk_t a_cbk) {
...
clib_error_code_t UTDataAVL_removeIf(UTDataAVL *a_avl,
  ...
  *a_result = ((void *)0);
  ...
  a_avl->root = UTDataAVLNode_removeIf(a_avl->root, a_cbk, a_context, a_result);
  if (*a_result != ((void *)0)) {
    return ((clib_error_code_t)0);
  }
  return ((clib_error_code_t)2);
...
clib_error_code_t UTDataAVL_removeNearOrEqual(UTDataAVL *a_avl,
  ...
  *a_res = ((void *)0);
  ...
  clib_error_code_t err = ((clib_error_code_t)0);
  const UTDataAVLNode *node = ((void *)0);
  do {
    err = UTDataAVL_findNearOrEqual(a_avl, a_value, &node);
    if (err != ((clib_error_code_t)0)) {
      break;
    }
